...
class HomeController extends GetxController {
  ...
  Future<void> getCurrentOrder() async {
    if (currentOrder.value.id != null && !driverModel.value.orderRequestData!.contains(currentOrder.value.id) && !driverModel.value.inProgressOrderID!.contains(currentOrder.value.id)) {
      currentOrder.value = OrderModel();
      await clearMap();
      await AudioPlayerService.playSound(false);
      update();
      ...
      if (driverModel.value.inProgressOrderID != null && driverModel.value.inProgressOrderID!.isNotEmpty) {
        FireStoreUtils.fireStore
            ...
            .where('id', isEqualTo: driverModel.value.inProgressOrderID?.first.toString())
            .snapshots()
            .listen(
              (event) async {
        ...
        FireStoreUtils.fireStore
            .collection(CollectionName.restaurantOrders)
            .where('status', whereNotIn: [Constant.orderCancelled, Constant.driverRejected])
            .where('id', isEqualTo: driverModel.value.orderRequestData!.first.toString())
            .snapshots()
            .listen(
              (event) async {
                if (event.docs.isNotEmpty) {
                  currentOrder.value = OrderModel.fromJson(event.docs.first.data());
                  if (driverModel.value.orderRequestData?.contains(currentOrder.value.id) == true) {
                    changeData();
                  } else {
                    currentOrder.value = OrderModel();
                    update();
  ...
  void animateToSource() {
    osmMapController.move(location.LatLng(driverModel.value.location!.latitude ?? 0.0, driverModel.value.location!.longitude ?? 0.0), 16);
  }
